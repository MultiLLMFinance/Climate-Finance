...
class ClimateNewsFinBERTPredictor:
    def __init__(self, csv_path, finbert_model="google-bert/bert-base-uncased", hf_token=None):
        """
        Initialize the stock predictor for climate change news analysis using BERT embeddings.
        
        Args:
            csv_path: Path to the CSV file containing climate change news data with merged news column
            finbert_model: The FinBERT model to use (default: "google-bert/bert-base-uncased")
        ...
        # Set max sequence length for merged news
        self.max_merged_length = 512  # Appropriate length for merged content
        
        # Create directories for visualizations
        self.cnn_viz_dir = 'Climate_news_second_database/Merged_BERT_CNN_LSTM/visualizations_cnn/COP'
        self.lstm_viz_dir = 'Climate_news_second_database/Merged_BERT_CNN_LSTM/visualizations_lstm/COP'
        os.makedirs(self.cnn_viz_dir, exist_ok=True)
        os.makedirs(self.lstm_viz_dir, exist_ok=True)
        os.makedirs('Climate_news_second_database/Merged_BERT_CNN_LSTM/visualizations_summary/COP', exist_ok=True)
        
        # Load FinBERT model and tokenizer
        self.load_finbert_model()
            
    def load_data(self):
        """Load and preprocess the CSV data containing climate change news with merged column."""
        print("Loading data...")
    ...
    def load_finbert_model(self):
        ...
        print(f"Loading BERT model: {self.finbert_model_name}")
    ...
    def preprocess_text_for_finbert(self, text):
        ...
        Returns:
            Preprocessed text ready for BERT tokenizer
        """
        if pd.isna(text):
            return ""
        
        # Convert to lowercase (BERT was trained on lowercase text)
        text = text.lower()
    ...
    def get_finbert_embeddings(self, texts, max_length):
        """Extract BERT embeddings with batch processing to reduce memory usage."""
    ...
    def split_data(self, layer, label_col, model_type):
        ...
        Extract BERT embeddings for merged news data.
        ...
        # Extract BERT embeddings for merged news
        print(f"Processing Merged news with max sequence length: {self.max_merged_length}")
    ...
    def create_cnn_model(self, input_shape):
        """
        Create a CNN model for text classification with BERT embeddings.
        Using the full text architecture since merged news will be longer.
        ...
        # CNN for merged text - using full text architecture since it's more robust
        # convolutional block
    ...
    def create_lstm_model(self, input_shape):
        """
        Create an LSTM model for text classification with BERT embeddings.
        Using the full text architecture since merged news will be longer.
        ...
        # Regular LSTM for merged text (using full text architecture)
        # LSTM layer
    ...
    def train_and_evaluate_deep_model(self, label_col, model_type):
        """
        Train and evaluate a deep learning model for the merged news column and a specific label column.
        ...
        """
        # Store results
        combination_key = f"{model_type}|{label_col}"
        ...
        print(f"\n{'='*80}")
        print(f"Training {model_type} model for Merged News and {label_col}")
        print(f"{'='*80}")
        ...
        for i, layer in enumerate(self.layers):
            ...
            print(f"Created {model_type} model for Merged News ({label_col})")
            model.summary()
            ...
            # Use batch size for full text since merged news will be longer
            batch_size = 16  
            history = model.fit(
    ...
    def run_all_combinations(self):
        """Run the analysis for all combinations of label columns and model types using Merged News."""
        ...
        for combination, results in self.results.items():
            model_type, label_col = combination.split('|')
            avg_accuracy = results.get('avg_accuracy', 0)
            avg_auc = results.get('avg_auc', 0)
            
            print(f"\nCombination: {model_type} with Merged News + {label_col}")
            ...
            summary_data.append({
                'Combination': f"{model_type} with Merged News + {label_col}",
    ...
    def create_summary_visualization(self, summary_data):
        """Create a summary visualization comparing all model combinations with Merged News."""
        if not summary_data:
        ...
        # 1. Performance comparison between all models
        ...
        plt.ylabel('Score')
        plt.title('Performance Comparison of Models with Merged News')
        ...
        save_path = os.path.join('Climate_news_second_database/Merged_BERT_CNN_LSTM/visualizations_summary/COP', "all_models_performance_comparison.png")
        plt.savefig(save_path)
        print(f"Overall performance comparison visualization saved as: {save_path}")
        plt.close()
        
        # 2. Layer-specific performance visualizations
        self.create_layer_performance_visualizations()
    
    def create_layer_performance_visualizations(self):
        """Create visualizations showing performance across different layers for models with Merged News."""
        ...
        for combination, results in self.results.items():
            ...
            for i, accuracy in enumerate(results.get('accuracy', [])):
                ...
                layer_info = {
                    ...
                    'Accuracy': accuracy,
                    'AUC': auc,
                    'Combination': f"{model_type} + {label_col}"
                }
                
                layer_data.append(layer_info)
        
        # Create visualization for all models
        self._create_layer_visualization(layer_data)
    
    def _create_layer_visualization(self, layer_data):
        """Create layer-specific visualizations for all models."""
        if not layer_data:
        ...
        plt.title('Accuracy by Model Combination and Layer with Merged News')
        ...
        plt.title('AUC by Model Combination and Layer with Merged News')
        ...
        # Save the visualization
        save_path = os.path.join('Climate_news_second_database/Merged_BERT_CNN_LSTM/visualizations_summary/COP', "layer_performance.png")
        plt.savefig(save_path)
...
if __name__ == "__main__":
    ...
    for directory in ['Climate_news_second_database/Merged_BERT_CNN_LSTM/visualizations_cnn/COP', 'Climate_news_second_database/Merged_BERT_CNN_LSTM/visualizations_lstm/COP', 
                      'Climate_news_second_database/Merged_BERT_CNN_LSTM/visualizations_summary/COP']:
        os.makedirs(directory, exist_ok=True)
    
    csv_path = '/home/c.c24004260/Climate_news_second_database/us_news_semantics_COP_completed.csv'
    
    # Hugging Face token (optional)
    hf_token = None  # Set to actual token if needed
    
    # Initialize the predictor with FinBERT model and token
    predictor = ClimateNewsFinBERTPredictor(csv_path, finbert_model="google-bert/bert-base-uncased", hf_token=hf_token)
    
    # Run the complete analysis
    predictor.load_data().define_time_windows().run_all_combinations()
    
    print("\nAnalysis complete! Results saved to 'Merged_BERT_CNN_LSTM' directory.")