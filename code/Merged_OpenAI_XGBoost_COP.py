...
class MergedEmbeddingStockPredictor:
    """
    Stock market trend prediction model using OpenAI merged embeddings
    """
    def __init__(self, csv_path):
        """
        Initialize the stock predictor using OpenAI merged embeddings.
        
        Args:
            csv_path: Path to the CSV file containing news data with pre-computed merged embeddings
        """
    ...
    def load_data(self):
        """
        Load and preprocess the CSV data containing news and merged embedding vectors.
        """
        ...
        # Parse merged embedding vectors from string to numpy arrays
    ...
    def process_embedding_vectors(self):
        """
        Convert the merged embedding vector strings to numpy arrays.
        """
        ...
        # Process merged embedding vectors
        if 'Merged_embedding_vector' in self.data.columns:
            print("Processing merged embedding vectors...")
            self.data['Merged_embedding'] = self.data['Merged_embedding_vector'].apply(parse_vector)
            # Verify dimensions
            sample_dim = len(self.data['Merged_embedding'].iloc[0])
            print(f"Merged embedding dimension: {sample_dim}")
        else:
            print("Error: Merged embedding vectors not found in dataset")
            raise ValueError("The dataset must contain a 'Merged_embedding_vector' column")
    ...
    def train_and_evaluate(self, label_col):
        """
        Train and evaluate the model for merged embeddings and a specific label column.
        
        Args:
            label_col: The label column to use ('S_label' or 'L_label')
        """
        # Store results
        combination_key = f"Merged|{label_col}"
        ...
        print(f"\n{'='*80}")
        print(f"Training XGBoost model for Merged embeddings and {label_col}")
        print(f"{'='*80}")
        ...
        for i, layer in enumerate(self.layers):
            ...
            print(f"Stage 1: Tuning core parameters for Merged embeddings...")
            ...
            print(f"Stage 2: Fine-tuning regularization parameters for Merged embeddings...")
    ...
    def plot_learning_curves(self, layer_result, label_col, layer_num):
        ...
        plt.suptitle(f"Learning Curves: Merged Embeddings + {label_col} (Layer {layer_num})", fontsize=16)
        ...
        viz_dir = f'Climate_news_second_database/Merged_OpenAI_XGBoost/visualizations/COP'
        os.makedirs(viz_dir, exist_ok=True)
        save_path = os.path.join(viz_dir, f"learning_curves_Merged_{label_col}_layer_{layer_num}.png")
        plt.savefig(save_path)
        print(f"Learning curves visualization saved as: {save_path}")
        plt.close()
    
    def run_all_combinations(self):
        """Run the analysis for all label columns using merged embeddings."""
    ...
    def create_summary_visualization(self, summary_data):
        ...
        plt.ylabel('Score')
        plt.title('Performance Comparison of Merged Embedding Models')
        ...
        os.makedirs('Climate_news_second_database/Merged_OpenAI_XGBoost/visualizations_summary/COP', exist_ok=True)
        save_path = os.path.join('Climate_news_second_database/Merged_OpenAI_XGBoost/visualizations_summary/COP', "performance_comparison.png")
        plt.savefig(save_path)
    ...
    def create_layer_performance_visualization(self):
        ...
        save_path = os.path.join('Climate_news_second_database/Merged_OpenAI_XGBoost/visualizations_summary/COP', "layer_performance_comparison.png")
        plt.savefig(save_path)
...
if __name__ == "__main__":
    ...
    for directory in ['Climate_news_second_database/Merged_OpenAI_XGBoost/visualizations/COP', 'Climate_news_second_database/Merged_OpenAI_XGBoost/visualizations_summary/COP']:
        os.makedirs(directory, exist_ok=True)
    
    # Path to your dataset with OpenAI merged embeddings
    ...
    predictor = MergedEmbeddingStockPredictor(dataset_path)
