...
class MergedNewsXGBoostPredictor:
    def __init__(self, csv_path):
        """
        Initialize the stock predictor for climate change news analysis with XGBoost,
        using merged news text (combined title and full text) as input.
    ...
    def load_data(self):
        ...
        # Check if "Merged news" column exists
        if "Merged news" not in self.data.columns:
            print("Warning: 'Merged news' column not found. Please ensure the CSV contains merged news data.")
        else:
            # Basic stats about merged news column
            merged_news_lengths = self.data["Merged news"].str.len()
            print(f"\nMerged news text statistics:")
            print(f"Average length: {merged_news_lengths.mean():.1f} characters")
            print(f"Min length: {merged_news_lengths.min()} characters")
            print(f"Max length: {merged_news_lengths.max()} characters")
    ...
    def split_data(self, layer, label_col):
        ...
        # Preprocess merged news text
        X_train = train_data["Merged news"].apply(self.preprocess_text)
        X_val = val_data["Merged news"].apply(self.preprocess_text)
        X_test = test_data["Merged news"].apply(self.preprocess_text)
    ...
    def get_tfidf_parameters(self):
        """
        Get TF-IDF parameters optimized for merged news text.
        The merged text contains both title and body, so parameters are adjusted accordingly.
        
        Returns:
            Dictionary of TF-IDF parameters
        """
        # Parameters are tuned for merged content, with considerations from both title and full text
        return {
            'max_features': 5000,   # Increased from full text (4000) to account for more content
            'min_df': 5,            # Moderate value between title (3) and full text (7)
            'max_df': 0.8,          # Moderate value between title (0.85) and full text (0.75)
            'ngram_range': (1, 3),  # Keep the broader range from full text
            'sublinear_tf': True    # Apply sublinear scaling
        }
    
    def create_model(self):
        """
        Create a TF-IDF + XGBoost pipeline model with anti-overfitting measures for merged news text.
        """
        # Get TF-IDF parameters for merged text
        tfidf_params = self.get_tfidf_parameters()
        ...
        # Define parameters for the first stage grid search
        # Using stronger anti-overfitting parameters due to merged text's larger feature space
        param_grid = {
            'classifier__n_estimators': [50, 100, 150],        # Range of tree counts
            'classifier__max_depth': [2, 3],                # Limited depth to prevent overfitting
            'classifier__learning_rate': [0.01, 0.03],   # Lower learning rates for stability
            'classifier__min_child_weight': [3, 5],            # Higher values to prevent overfitting
            'classifier__subsample': [0.7, 0.8],               # Row subsampling to reduce overfitting
            'classifier__colsample_bytree': [0.6, 0.7]         # Feature subsampling to reduce overfitting
        }
    ...
    def create_second_stage_model(self, best_params):
        """
        Create a second stage model to fine-tune regularization parameters.
        ...
        learning_rate = best_params.get('classifier__learning_rate', 0.03)
        min_child_weight = best_params.get('classifier__min_child_weight', 3)
        subsample = best_params.get('classifier__subsample', 0.8)
        colsample_bytree = best_params.get('classifier__colsample_bytree', 0.7)
        ...
        # Strong regularization for merged text to combat overfitting
        param_grid = {
            'classifier__gamma': [0.1, 0.3, 0.5],         # Minimum loss reduction for splitting
            'classifier__reg_alpha': [0.5, 1.0, 2.0],     # L1 regularization
            'classifier__reg_lambda': [1.0, 3.0, 5.0]     # L2 regularization
        }
    ...
    def train_and_evaluate(self, label_col):
        """
        Train and evaluate the model for a specific label column using merged news text.
        
        Args:
            label_col: The label column to use ('S_label' or 'L_label')
        """
        # Store results
        combination_key = f"Merged news|{label_col}"
        ...
        print(f"\n{'='*80}")
        print(f"Training XGBoost model for Merged news and {label_type} prediction")
        print(f"{'='*80}")
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Split data
            (X_train, y_train), (X_val, y_val), (X_test, y_test), train_data, val_data, test_data = self.split_data(layer, label_col)
            ...
            # Stage 1: Create and train model with core parameters
            print(f"Stage 1: Tuning core parameters for merged news...")
            model_stage1 = self.create_model()
            ...
            print(f"Stage 2: Fine-tuning regularization parameters for merged news...")
            model_stage2 = self.create_second_stage_model(best_params_stage1)
            ...
            self.plot_learning_curves(layer_result, label_col, i+1)
    ...
    def plot_learning_curves(self, layer_result, label_col, layer_num):
        """Plot learning curves to detect overfitting with improved styling."""
        plt.figure(figsize=(15, 10))
        
        label_type = "Short-term" if label_col == "S_label" else "Long-term"
        plt.suptitle(f"Learning Curves: Merged News + {label_type} (Layer {layer_num})", fontsize=16)
        ...
        viz_dir = f'Climate_news_second_database/Merged_News_TFIDF_XGBoost/visualizations/COP'
        os.makedirs(viz_dir, exist_ok=True)
        
        save_path = os.path.join(viz_dir, f"learning_curves_{label_type}_layer_{layer_num}.png")
        plt.savefig(save_path)
        print(f"Learning curves visualization saved as: {save_path}")
        plt.close()
    
    def run_all_combinations(self):
        """Run the analysis for all label columns using merged news."""
        # Define all combinations - now just the two label columns with merged news
        label_columns = ['S_label', 'L_label']
        
        # Run analysis for each label column
        for label_col in label_columns:
            self.train_and_evaluate(label_col)
        ...
        for combination, results in self.results.items():
            ...
            print(f"\nCombination: {text_col} + {label_type}")
            ...
            summary_data.append({
                'Combination': f"{text_col} + {label_type}",
    ...
    def create_summary_visualization(self, summary_data):
        ...
        plt.ylabel('Score')
        plt.title('Performance Comparison of Merged News Models')
        ...
        os.makedirs('Climate_news_second_database/Merged_News_TFIDF_XGBoost/visualizations_summary/COP', exist_ok=True)
        save_path = os.path.join('Climate_news_second_database/Merged_News_TFIDF_XGBoost/visualizations_summary/COP', "performance_comparison.png")
        plt.savefig(save_path)
    ...
    def create_layer_performance_visualization(self):
        ...
        save_path = os.path.join('Climate_news_second_database/Merged_News_TFIDF_XGBoost/visualizations_summary/COP', "layer_performance_comparison.png")
        plt.savefig(save_path)
...
if __name__ == "__main__":
    ...
    for directory in ['Climate_news_second_database/Merged_News_TFIDF_XGBoost/visualizations/COP', 'Climate_news_second_database/Merged_News_TFIDF_XGBoost/visualizations_summary/COP']:
        os.makedirs(directory, exist_ok=True)
    
    predictor = MergedNewsXGBoostPredictor('/home/c.c24004260/Climate_news_second_database/us_news_semantics_COP_completed.csv')
