...
class PlotLearningCallback(Callback):
    def __init__(self, model_type, label_col, layer_num, plot_dir):
        super().__init__()
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch % 5 == 0 or epoch == self.params['epochs'] - 1:  # Plot every 5 epochs and last epoch
            ...
            ax1.set_title(f'Accuracy Curves: {self.model_type} with Merged News ({self.label_col}, Layer {self.layer_num})')
            ...
            ax2.set_title(f'Loss Curves: {self.model_type} with Merged News ({self.label_col}, Layer {self.layer_num})')
            ...
            plt.tight_layout()
            plt.savefig(f"{self.plot_dir}/{self.model_type}_merged_news_{self.label_col}_layer_{self.layer_num}_epoch_{epoch}.png")
            plt.close()

class MergedNewsFinBERTPredictor:
    def __init__(self, csv_path, finbert_model="yiyanghkust/finbert-esg-9-categories", hf_token=None):
        """
        Initialize the stock predictor for climate change news analysis using FinBERT-ESG embeddings
        with merged news (title + full text) as input.
        
        Args:
            csv_path: Path to the CSV file containing merged climate change news data
        ...
        # Set sequence length for merged news - respect BERT's 512 token limitation
        # Accounting for special tokens [CLS] and [SEP]
        self.max_merged_length = 512
        
        # Create directories for visualizations with "Merged" prefix
        self.cnn_viz_dir = 'Climate_news_second_database/Merged_FinBERT_Y_CNN_LSTM/visualizations_cnn/COP'
        self.lstm_viz_dir = 'Climate_news_second_database/Merged_FinBERT_Y_CNN_LSTM/visualizations_lstm/COP'
        os.makedirs(self.cnn_viz_dir, exist_ok=True)
        os.makedirs(self.lstm_viz_dir, exist_ok=True)
        os.makedirs('Climate_news_second_database/Merged_FinBERT_Y_CNN_LSTM/visualizations_summary/COP', exist_ok=True)
        
        # Load FinBERT model and tokenizer
        self.load_finbert_model()
            
    def load_data(self):
        """Load and preprocess the CSV data containing merged climate change news."""
        print("Loading data...")
        self.data = pd.read_csv(self.csv_path)
        
        # Check if the merged news column exists
        if "Merged news" not in self.data.columns:
            raise ValueError("The 'Merged news' column is not found in the CSV file. Please ensure you're using the correct file.")
        ...
        # Analyze the merged news text characteristics
        merged_news_lengths = self.data["Merged news"].astype(str).str.len()
        token_counts = self.data["Merged news"].astype(str).apply(lambda x: len(nltk.word_tokenize(x)))
        
        print("\nMerged news statistics:")
        print(f"Character length - Min: {merged_news_lengths.min()}, Max: {merged_news_lengths.max()}, Mean: {merged_news_lengths.mean():.1f}")
        print(f"Token count - Min: {token_counts.min()}, Max: {token_counts.max()}, Mean: {token_counts.mean():.1f}, Median: {token_counts.median()}")
        print(f"Using maximum sequence length of {self.max_merged_length} tokens for merged news (FinBERT limitation)")
        
        # Report potential truncation impact
        tokens_exceeding = (token_counts > self.max_merged_length).sum()
        if tokens_exceeding > 0:
            print(f"Warning: {tokens_exceeding} articles ({tokens_exceeding/len(self.data)*100:.1f}%) exceed the {self.max_merged_length} token limit and will be truncated")
        
        return self
    
    def load_finbert_model(self):
        """Load the FinBERT model and tokenizer."""
        print(f"Loading FinBERT-ESG model: {self.finbert_model_name}")
        ...
        if torch.cuda.is_available():
            ...
            print("FinBERT-ESG model loaded on GPU")
        else:
            print("GPU not available, running on CPU")
            
        print("FinBERT-ESG model and tokenizer loaded successfully")
        return self
    ...
    def get_finbert_embeddings(self, texts, max_length):
        ...
        # Use smaller batches for merged news to avoid OOM errors
        batch_size = 50  # Smaller batches for longer merged texts
    ...
    def split_data(self, layer, label_col, model_type):
        """
        Split the data into training, validation, and test sets based on the defined time windows.
        Extract FinBERT embeddings for merged news data.
        ...
        # Extract FinBERT embeddings for merged news
        print("Extracting FinBERT embeddings for training data...")
        X_train_embeddings, X_train_mask = self.get_finbert_embeddings(train_data["Merged news"].tolist(), self.max_merged_length)
        
        print("Extracting FinBERT embeddings for validation data...")
        X_val_embeddings, X_val_mask = self.get_finbert_embeddings(val_data["Merged news"].tolist(), self.max_merged_length)
        
        print("Extracting FinBERT embeddings for test data...")
        X_test_embeddings, X_test_mask = self.get_finbert_embeddings(test_data["Merged news"].tolist(), self.max_merged_length)
    ...
    def create_cnn_model(self, input_shape):
        """
        Create a CNN model optimized for merged news classification with FinBERT embeddings.
        ...
        Returns:
            Compiled CNN model
        """
        print("Creating CNN model on CPU for merged news...")
        
        inputs = Input(shape=input_shape)
        
        # CNN architecture optimized for merged news
        ...
        model.compile(
            optimizer=Adam(learning_rate=0.00005),  # Balance between title and full text rates
    ...
    def create_lstm_model(self, input_shape):
        """
        Create an LSTM model optimized for merged news classification with FinBERT embeddings.
        ...
        Returns:
            Compiled LSTM model
        """
        print("Creating LSTM model on CPU for merged news...")
        
        inputs = Input(shape=input_shape)
        
        # LSTM architecture optimized for merged news
    ...
    def plot_final_learning_curves(self, history, model_type, label_col, layer_num, visualization_dir):
        """
        ...
        ax1.set_title(f'Final Accuracy Curves: {model_type} with Merged News ({label_col}, Layer {layer_num})')
        ax1.set_xlabel('Epochs')
        ax1.set_ylabel('Accuracy')
        ax1.legend(loc='lower right')
        ax1.set_ylim([0, 1])
        ...
        ax2.set_title(f'Final Loss Curves: {model_type} with Merged News ({label_col}, Layer {layer_num})')
        ax2.set_xlabel('Epochs')
        ax2.set_ylabel('Loss')
        ax2.legend(loc='upper right')
        ax2.grid(True, linestyle='--', alpha=0.7)
        ...
        plt.tight_layout()
        filename = f"final_{model_type.lower()}_merged_news_{label_col}_layer_{layer_num}.png"
        plt.savefig(f"{visualization_dir}/{filename}")
        print(f"Saved learning curves: {filename}")
        plt.close()
    
    def train_and_evaluate_deep_model(self, label_col, model_type):
        """
        Train and evaluate a deep learning model for merged news with a specific label column.
        ...
        """
        # Store results
        combination_key = f"{model_type}|Merged news|{label_col}"
        ...
        print(f"\n{'='*80}")
        print(f"Training {model_type} model for Merged news and {label_type} prediction")
        print(f"{'='*80}")
        ...
        for i, layer in enumerate(self.layers):
            ...
            (X_train_embeddings, X_train_mask, y_train), (X_val_embeddings, X_val_mask, y_val), \
            (X_test_embeddings, X_test_mask, y_test), train_data, val_data, test_data = self.split_data(
                layer, label_col, model_type)
            ...
            print(f"Created {model_type} model for Merged news ({label_type})")
            model.summary()
            
            # Setup callbacks
            plot_callback = PlotLearningCallback(model_type, label_col, i+1, visualization_dir)
            ...
            model_checkpoint = ModelCheckpoint(
                filepath=f"{visualization_dir}/best_{model_type.lower()}_merged_news_{label_col}_layer_{i+1}.weights.h5",
                monitor='val_loss',
            ...
            # Using smaller batch size for merged news to manage memory
            batch_size = 16
            history = model.fit(
            ...
            self.plot_final_learning_curves(history, model_type, label_col, i+1, visualization_dir)
    ...
    def run_all_combinations(self):
        """Run the analysis for merged news with all label columns and model types."""
        ...
        for model_type in model_types:
            for label_col in label_cols:
                self.train_and_evaluate_deep_model(label_col, model_type)
        ...
        for combination, results in self.results.items():
            ...
            print(f"\nCombination: {model_type} with {text_col} + {label_type}")
            ...
            summary_data.append({
                'Combination': f"{model_type} with {text_col} + {label_type}",
    ...
    def create_summary_visualization(self, summary_data):
        ...
        # Create combined visualization for all models
        ...
        plt.ylabel('Score')
        plt.title('Performance Comparison of Models with Merged News and FinBERT-ESG')
        labels = [f"{row['Model']} + {row['Label']}" for _, row in df.iterrows()]
        ...
        save_path = os.path.join('Climate_news_second_database/Merged_FinBERT_Y_CNN_LSTM/visualizations_summary/COP', "overall_performance_comparison.png")
        plt.savefig(save_path)
        print(f"Overall summary comparison visualization saved as: {save_path}")
        plt.close()
        ...
        # Create separate visualizations for each model type if available
        if len(cnn_data) > 0:
            self._create_model_type_visualization(cnn_data, "CNN")
        
        if len(lstm_data) > 0:
            self._create_model_type_visualization(lstm_data, "LSTM")
        
        # Create layer-specific performance visualizations
        self.create_layer_performance_visualizations()
    
    def _create_model_type_visualization(self, data, model_type):
        """Create a visualization specific to one model type."""
        ...
        plt.bar(x - width/2, data['Avg Accuracy'], width, label='Average Accuracy', color='skyblue')
        plt.bar(x + width/2, data['Avg AUC'], width, label='Average AUC', color='salmon')
        
        plt.xlabel('Prediction Horizon')
        plt.ylabel('Score')
        plt.title(f'Performance of {model_type} Models with Merged News and FinBERT-ESG')
        labels = ["Short-term" if "S_label" in row['Label'] else "Long-term" for _, row in data.iterrows()]
        plt.xticks(x, labels, rotation=0)
        plt.legend()
        
        # Add value labels on top of bars
        for i, v in enumerate(data['Avg Accuracy']):
            plt.text(i - width/2, v + 0.01, f"{v:.3f}", ha='center', va='bottom', fontsize=10)
        
        for i, v in enumerate(data['Avg AUC']):
            plt.text(i + width/2, v + 0.01, f"{v:.3f}", ha='center', va='bottom', fontsize=10)
        
        plt.ylim(0, max(data['Avg Accuracy'].max(), data['Avg AUC'].max()) + 0.1)
        plt.tight_layout()
        
        # Save the visualization
        save_path = os.path.join('Climate_news_second_database/Merged_FinBERT_Y_CNN_LSTM/visualizations_summary/COP', f"{model_type.lower()}_performance_comparison.png")
        plt.savefig(save_path)
        print(f"{model_type} summary comparison visualization saved as: {save_path}")
        plt.close()
    
    def create_layer_performance_visualizations(self):
        """Create visualizations showing performance across different layers for all models."""
        ...
        for combination, results in self.results.items():
            ...
            for i, accuracy in enumerate(results.get('accuracy', [])):
                ...
                layer_info = {
                    ...
                    'Accuracy': accuracy,
                    'AUC': auc,
                    'Combination': f"{model_type} + {label_type}"
                }
        ...
        if not layer_data:
            print("No layer data available for visualization")
            return
        ...
        self._create_layer_visualization(df, "All Models")
        
        # Also create separate visualizations for CNN and LSTM
        cnn_df = df[df['Model'] == 'CNN']
        lstm_df = df[df['Model'] == 'LSTM']
        
        if len(cnn_df) > 0:
            self._create_layer_visualization(cnn_df, "CNN")
            
        if len(lstm_df) > 0:
            self._create_layer_visualization(lstm_df, "LSTM")
    
    def _create_layer_visualization(self, df, title_prefix):
        """Create layer-specific visualizations for given data."""
        ...
        plt.title(f'{title_prefix} Accuracy by Combination and Layer with Merged News and FinBERT-ESG')
        ...
        plt.title(f'{title_prefix} AUC by Combination and Layer with Merged News and FinBERT-ESG')
        ...
        save_path = os.path.join('Climate_news_second_database/Merged_FinBERT_Y_CNN_LSTM/visualizations_summary/COP', f"{filename}_layer_performance.png")
        plt.savefig(save_path)
        print(f"{title_prefix} layer performance visualization saved as: {save_path}")
        plt.close()
...
if __name__ == "__main__":
    ...
    for directory in ['Climate_news_second_database/Merged_FinBERT_Y_CNN_LSTM/visualizations_cnn/COP', 
                     'Climate_news_second_database/Merged_FinBERT_Y_CNN_LSTM/visualizations_lstm/COP', 
                     'Climate_news_second_database/Merged_FinBERT_Y_CNN_LSTM/visualizations_summary/COP']:
    ...
    # Initialize the predictor with FinBERT-ESG model and token
    predictor = MergedNewsFinBERTPredictor(csv_path, finbert_model="yiyanghkust/finbert-esg-9-categories", hf_token=hf_token)
    
    # Run the complete analysis
    predictor.load_data().define_time_windows().run_all_combinations()
    
    print("\nAnalysis complete! Results saved to 'Merged_FinBERT_ESG_CNN_LSTM' directory.")