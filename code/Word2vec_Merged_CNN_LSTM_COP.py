...
class MergedNewsDeepLearningPredictor:
    def __init__(self, csv_path, word2vec_path='/home/c.c24004260/models/word2vec-google-news-300.bin'):
        """
        Initialize the stock predictor for merged climate change news analysis with deep learning models.
        ...
        # Define sequence length for merged news
        # This is a balance between title and full text - we want to capture the title
        # and as much meaningful content as possible while managing memory constraints
        self.max_merged_length = 520  # Slightly larger than full text to accommodate title
        
        # Create directories for visualizations
        self.cnn_viz_dir = 'Climate_news_second_database/Merged_Word2vec_CNN_LSTM/visualizations_cnn/COP'
        self.lstm_viz_dir = 'Climate_news_second_database/Merged_Word2vec_CNN_LSTM/visualizations_lstm/COP'
        os.makedirs(self.cnn_viz_dir, exist_ok=True)
        os.makedirs(self.lstm_viz_dir, exist_ok=True)
        os.makedirs('Climate_news_second_database/Merged_Word2vec_CNN_LSTM/visualizations_summary/COP', exist_ok=True)
    ...
    def load_data(self):
        ...
        # Analyze the length of the merged news text
        merged_news_lengths = self.data["Merged news"].astype(str).str.len()
        token_lengths = self.data["Merged news"].astype(str).apply(lambda x: len(word_tokenize(x)))
        ...
        print(f"Token length - Min: {token_lengths.min()}, Max: {token_lengths.max()}, Mean: {token_lengths.mean():.1f}, Median: {token_lengths.median():.1f}")
        print(f"Using maximum sequence length of {self.max_merged_length} tokens for merged news")
    ...
    def split_data(self, layer, label_col, model_type):
        ...
        Process merged news text using Word2Vec embeddings and create fixed-length sequences.
        ...
        if model_type == 'CNN' or model_type == 'LSTM':
            # Using the merged news sequence length
            max_length = self.max_merged_length
            
            print(f"Processing Merged news with max sequence length: {max_length}")
            
            # Preprocess and tokenize text
            X_train_tokens = [self.preprocess_text_for_word2vec(text) for text in train_data["Merged news"]]
            X_val_tokens = [self.preprocess_text_for_word2vec(text) for text in val_data["Merged news"]]
            X_test_tokens = [self.preprocess_text_for_word2vec(text) for text in test_data["Merged news"]]
            
            # Analysis of token lengths
            train_token_lengths = [len(tokens) for tokens in X_train_tokens]
            print(f"Token length stats for Merged news (train set):")
            ...
            print(f"  Percentage truncated: {sum(1 for l in train_token_lengths if l > max_length) / len(train_token_lengths) * 100:.1f}%")
    ...
    def create_cnn_model(self):
        """
        Create a CNN model for merged news classification with Word2Vec embeddings.
        The architecture is optimized for the merged news context.
        """
        # Input shape based on merged news sequence length
        input_shape = (self.max_merged_length, 300)
        
        # Create CNN model for merged news
        model = Sequential()
        
        # Convolutional layers - Using a moderately complex architecture
        # We need more capacity than title-only but with regularization to prevent overfitting
        model.add(Conv1D(filters=128, kernel_size=7, activation='relu', input_shape=input_shape))
        model.add(MaxPooling1D(pool_size=2))
        model.add(Dropout(0.5))  # Moderate dropout
        
        # Flatten and dense layers
        model.add(GlobalMaxPooling1D())
        model.add(Dense(64, activation='relu'))
        model.add(Dropout(0.5))  # Stronger dropout for dense layers
    ...
    def create_lstm_model(self):
        """
        Create an LSTM model for merged news classification with Word2Vec embeddings.
        The architecture is optimized for the merged news context.
        """
        # Input shape based on merged news sequence length
        input_shape = (self.max_merged_length, 300)
        
        # Create LSTM model for merged news
        model = Sequential()
        
        # Using a bidirectional LSTM to better capture context in both directions
        model.add(Bidirectional(LSTM(64, dropout=0.3, recurrent_dropout=0.3), input_shape=input_shape))
        
        # Dense layers
        model.add(Dense(32, activation='relu'))
        model.add(Dropout(0.5))  # Strong dropout to prevent overfitting
        model.add(Dense(1, activation='sigmoid'))
    ...
    def train_and_evaluate_deep_model(self, label_col, model_type):
        ...
        for i, layer in enumerate(self.layers):
            ...
            early_stopping = EarlyStopping(
                monitor='val_loss',
                patience=7,  # Using the same patience as the original script
                restore_best_weights=True,
                verbose=1
            )
            model_checkpoint = ModelCheckpoint(
                filepath=f"{visualization_dir}/best_{model_type.lower()}_merged_news_{label_col}_layer_{i+1}.h5",
                monitor='val_loss',
                save_best_only=True,
                verbose=1
            )
            ...
            history = model.fit(
                ...
                epochs=50,  # Same as original script
    ...
    def create_summary_visualization(self, summary_data):
        ...
        # Plot accuracy and AUC bars for all models
        x = np.arange(len(df))
        width = 0.35
        ...
        plt.ylabel('Score')
        plt.title('Performance Comparison of All Models on Merged News')
        plt.xticks(x, [f"{row['Model']} + {row['Label']}" for _, row in df.iterrows()], rotation=0)
        plt.legend()
        ...
        save_path = os.path.join('Climate_news_second_database/Merged_Word2vec_CNN_LSTM/visualizations_summary/COP', "overall_performance_comparison.png")
        plt.savefig(save_path)
        print(f"Overall summary comparison visualization saved as: {save_path}")
        plt.close()
        
        # Create separate visualizations for CNN and LSTM for clarity
        if len(cnn_data) > 0:
    ...
    def _create_model_type_visualization(self, data, model_type):
        ...
        plt.ylabel('Score')
        plt.title(f'Performance of {model_type} Models on Merged News')
        plt.xticks(x, [row['Label'] for _, row in data.iterrows()], rotation=0)
        plt.legend()
        ...
        # Save the visualization
        save_path = os.path.join('Climate_news_second_database/Merged_Word2vec_CNN_LSTM/visualizations_summary/COP', f"{model_type.lower()}_performance_comparison.png")
        plt.savefig(save_path)
    ...
    def _create_layer_visualization(self, df, title_prefix):
        ...
        plt.title(f'{title_prefix} Accuracy by Combination and Layer')
        ...
        plt.title(f'{title_prefix} AUC by Combination and Layer')
        ...
        save_path = os.path.join('Climate_news_second_database/Merged_Word2vec_CNN_LSTM/visualizations_summary/COP', f"{filename}_layer_performance.png")
        plt.savefig(save_path)
...
if __name__ == "__main__":
    ...
    for directory in ['Climate_news_second_database/Merged_Word2vec_CNN_LSTM/visualizations_cnn/COP', 
                      'Climate_news_second_database/Merged_Word2vec_CNN_LSTM/visualizations_lstm/COP', 
                      'Climate_news_second_database/Merged_Word2vec_CNN_LSTM/visualizations_summary/COP', 
                      'models']:
    ...
    predictor = MergedNewsDeepLearningPredictor(csv_path, word2vec_path)
