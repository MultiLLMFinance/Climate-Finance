...
def focal_loss(gamma=2.0, alpha=0.25):
    ...
    This loss function reduces the relative contribution of well-classified examples
    and focuses more on hard, misclassified examples during training.
    ...
    Args:
        ...
        alpha: Class weight parameter for class imbalance. 
            ...
            Values closer to 0 or 1 give more weight to one class.
...
class Fino1MergedSemanticVectorPredictor:
    def __init__(self, csv_path):
        """
        Initialize the stock predictor for Fino1 merged semantic vector analysis.
        
        Args:
            csv_path: Path to the CSV file containing Fino1 merged semantic vectors and stock labels
        """
        ...
        # Define merged semantic vector column - this is the exact column name in the CSV
        self.vector_columns = {
            'merged': 'Fino1_merged'
        }
        ...
        os.makedirs('Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/visualizations/COP/learning_curves', exist_ok=True)
        os.makedirs('Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/visualizations/COP/performance_comparisons', exist_ok=True)
        os.makedirs('Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/visualizations/COP/layer_comparisons', exist_ok=True)
        os.makedirs('Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/visualizations/COP/confusion_matrices', exist_ok=True)
        
        # Create directory for Fino1 learning curves
        os.makedirs('Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/visualizations/COP/learning_curves/Fino1', exist_ok=True)
        
        # Create directory for model checkpoints
        os.makedirs('Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/Best_model/COP', exist_ok=True)
    ...
    def split_data(self, layer, vector_column, label_column):
        ...
        Returns:
            Split datasets and corresponding indices, and optimal alpha for focal loss
        """
    ...
    def create_sequence_data(self, X, y, indices, window_size=10):
        ...
        Returns:
            X_seq, y_seq: Sequences for LSTM input and corresponding targets, and optimal alpha
        """
        X_seq, y_seq = [], []
    ...
    def train_and_evaluate(self, vector_column, label_column, window_size=10,
        ...
        """
        # Using "Merged" as the text type for the merged vectors
        text_type = 'Merged'
        ...
        print(f"\n{'='*80}")
        print(f"Training model for Fino1 approach, {text_type}, {label_type}, window size {window_size}")
        ...
        for i, layer in enumerate(self.layers):
            ...
            callbacks = [
                ...
                ModelCheckpoint(
                    f'Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/Best_model/COP/model_Fino1_SemanticVectors_{text_type}_{label_type}_focal_layer{i+1}_w{window_size}.h5',
                    monitor='val_loss',
                    save_best_only=True
                )
            ]
    ...
    def visualize_confusion_matrix(self, cm, approach, text_type, label_type, layer_num, 
                                  window_size=None, focal_alpha=None, focal_gamma=None):
        """
        ...
        Args:
            ...
            text_type: Text type (Merged)
            label_type: Label type (Short-term or Long-term)
            layer_num: Layer number
            window_size: Window size used for this model
        ...
        # Add parameters to title if provided
        window_str = f", W={window_size}" if window_size else ""
        focal_params = ''
        if focal_alpha is not None and focal_gamma is not None:
            focal_params = f", α={focal_alpha:.2f}, γ={focal_gamma:.1f}"
        
        plt.title(f"Confusion Matrix: {approach} Merged Semantic Vectors - {label_type}\nLayer {layer_num}{window_str}{focal_params}")
        plt.ylabel('True Label')
        plt.xlabel('Predicted Label')
        
        # Save figure
        save_path = f'Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/visualizations/COP/confusion_matrices/Fino1_{text_type}_{label_type}_layer{layer_num}_w{window_size}_focal.png'
    ...
    def visualize_learning_curves(self, history, approach, text_type, label_type, layer_num,
        ...
        plt.suptitle(f"Learning Curves: {approach} Merged Semantic Vectors - {label_type}\nLayer {layer_num}{window_str}{focal_params}", 
                   fontsize=16)
        ...
        window_suffix = f"_w{window_size}" if window_size else ""
        focal_suffix = "_focal" if focal_alpha is not None else ""
        
        save_path = f'Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/visualizations/COP/learning_curves/{approach}/{text_type}_{label_type}_SemanticVectors_layer{layer_num}{window_suffix}{focal_suffix}.png'
        plt.savefig(save_path)
        plt.close()
    ...
    def run_all_combinations(self, perform_grid_search=True, focal_gamma=2.0):
        """
        Run the analysis for all combinations of Fino1 merged semantic vector column and label columns.
        
        Args:
            perform_grid_search: Whether to perform grid search for window size first
            focal_gamma: Gamma parameter for focal loss
        """
        # Define all combinations for Fino1 merged semantic vector column
        combinations = []
        ...
        for vector_col, label_col in combinations:
            ...
            if perform_grid_search:
                ...
                if grid_result and 'best_model_results' in grid_result and grid_result['best_model_results']:
                    ...
                    result_text_type = 'Merged'  # Always 'Merged' for merged vectors
        ...
        if perform_grid_search:
            ...
            for (vector_col, label_col), params in best_params.items():
                text_type = 'Merged'  # Always 'Merged' for merged vectors
    ...
    def _create_performance_by_approach_visualizations(self):
        """Create visualizations comparing performance by prediction timeframe."""
        # Collect data for Fino1 merged semantic vectors
        combinations = []
        accuracies = []
        aucs = []
        f1_scores = []
        ...
        for label_type in ['Short-term', 'Long-term']:
            ...
            if combination_key in self.results and 'avg_accuracy' in self.results[combination_key]:
                ...
                # Get window size and focal alpha if available
        ...
        plt.title('Accuracy by Model Combination - Fino1 Merged Semantic Vectors')
        plt.ylim(0, 1.0)
        ...
        for i, p in enumerate(ax.patches):
            ax.annotate(f"W: {df['Window Size'][i]}", 
                       ...
                       ha = 'center', va = 'bottom',
                       xytext = (0, 5), textcoords = 'offset points')
        plt.title('AUC by Model Combination - Fino1 Merged Semantic Vectors')
        plt.ylim(0, 1.0)
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        ...
        for i, p in enumerate(ax.patches):
            ax.annotate(f"α: {df['Focal Alpha'][i]}", 
                       ...
                       ha = 'center', va = 'bottom',
                       xytext = (0, 5), textcoords = 'offset points')
        plt.title('F1 Score by Model Combination - Fino1 Merged Semantic Vectors')
        plt.ylim(0, 1.0)
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        ...
        plt.title('Precision by Model Combination - Fino1 Merged Semantic Vectors')
        plt.ylim(0, 1.0)
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        ...
        plt.title('Recall by Model Combination - Fino1 Merged Semantic Vectors')
        plt.ylim(0, 1.0)
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        ...
        plt.tight_layout()
        plt.savefig('Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/visualizations/COP/performance_comparisons/metrics_Fino1_Merged_SemanticVectors_focal.png')
        plt.close()
    ...
    def _create_performance_by_layer_visualizations(self):
        ...
        for label_type in ['Short-term', 'Long-term']:
            ...
            if combination_key in self.results:
                combination = f"Merged-{label_type}"
        ...
        plt.title('Accuracy by Layer and Combination - Fino1 Merged Semantic Vectors')
        plt.ylim(0, 1.0)
        ...
        plt.title('AUC by Layer and Combination - Fino1 Merged Semantic Vectors')
        plt.ylim(0, 1.0)
        plt.legend(title='Combination', bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        ...
        plt.title('F1 Score by Layer and Combination - Fino1 Merged Semantic Vectors')
        plt.ylim(0, 1.0)
        plt.legend(title='Combination', bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        ...
        plt.title('Precision by Layer and Combination - Fino1 Merged Semantic Vectors')
        plt.ylim(0, 1.0)
        plt.legend(title='Combination', bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        ...
        plt.title('Recall by Layer and Combination - Fino1 Merged Semantic Vectors')
        plt.ylim(0, 1.0)
        plt.legend(title='Combination', bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(axis='y', linestyle='--', alpha=0.7)
        
        plt.tight_layout()
        plt.savefig('Climate_news_second_database/Merged_Fino1_semantic_LSTM_Plots/visualizations/COP/layer_comparisons/metrics_by_layer_Fino1_Merged_SemanticVectors_focal.png')
        plt.close()
    
    def print_summary(self):
        """Print a summary of all results."""
        print("\n" + "="*80)
        print("SUMMARY OF RESULTS FOR FINO1 MERGED SEMANTIC VECTORS WITH FOCAL LOSS (COP)")
        print("="*80)
        ...
        for label_type in ['Short-term', 'Long-term']:
            ...
            if combination_key in self.results and 'avg_accuracy' in self.results[combination_key]:
                ...
                print(f"Merged + {label_type} (Window Size: {window_size}, Focal α: {focal_alpha}, γ: {focal_gamma}):")
...
if __name__ == "__main__":
    ...
    if is_jupyter:
        ...
        parser = argparse.ArgumentParser(description='Stock trend prediction using Fino1 merged semantic vectors with focal loss')
    ...
    predictor = Fino1MergedSemanticVectorPredictor('/home/c.c24004260/Climate_news_second_database/us_news_semantics_COP_completed_vectors_Fino1_Merged.csv')
